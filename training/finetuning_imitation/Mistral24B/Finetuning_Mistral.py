...
class MetricsLoggingCallback(TrainerCallback):
    def __init__(self, file_path="metrics_log_QLORA.csv"):
...
class FineTuneConfig:
    ...
    """
    # On remplace le modèle Phi-4-mini-instruct par mistralai/Mistral-Small-24B-Instruct-2501
    # et on précise qu'on va le stocker dans un dossier local 'model'
    ...
    output_dir: str = field(
        default="./checkpoint_trump_QLORA",
...
class FineTuner:
    ...
    def load_model_and_tokenizer(self):
        ...
        # On prépare la configuration pour QLoRA en 4 bits
        bnb_config = BitsAndBytesConfig(
            load_in_4bit=True,
            bnb_4bit_use_double_quant=True,
            bnb_4bit_quant_type="nf4",
            # Vous pouvez ajuster le dtype si nécessaire (float16, bfloat16, etc.)
            bnb_4bit_compute_dtype=torch.float16
        )
        
        # On prépare les kwargs du modèle
        model_kwargs = {
        ...
        # Préparer le modèle pour kbit training (QLoRA)
    ...
    def create_trainer(self):
        ...
        self.trainer = SFTTrainer(
            ...
            callbacks=[MetricsLoggingCallback("metrics_log_QLORA.csv"), EmptyCacheCallback()]
        )
...
def main():
    config = FineTuneConfig(
        ...
        output_dir="./checkpoint_trump_QLORA",
        num_train_epochs=1,
